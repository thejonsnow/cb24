...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  // Add the event listener for the button with ID 'shitty'
  document.getElementById("shitty").addEventListener("click", async () => {
    const events = await fetchEventsWithSfeTrue();
    populateEvents(events, true);

    // Update the "Showing events for" text to "Showing Shitty Events"
    const showingForTextBox = document.getElementById("showingForTextBox");
    showingForTextBox.innerHTML = "Showing Shitty Events";
  });
  ...
  async function fetchEventsWithSfeTrue() {
  const eventsRef = collection(db, "events");
  const q = query(eventsRef, where("sfe", "==", true));
  ...
  querySnapshot.forEach((doc) => {
    events.push({
      ...
      img: doc.data().thumbnail,
      location: doc.data().location,
      description: doc.data().description || "No event description here.",
      price: doc.data().ticketPrice,
      tickets: doc.data().url,
      date: doc.data().date,
    });
