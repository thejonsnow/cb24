  ...
  function queryAllDocs(queryS) {
    ...
    queryS.forEach((doc) => {
      shows.push({
        ...
        description: doc.data().description || "No event description here, but we all know you've made worse decisions with less information, so this shouldn't stop you.",
    ...
    searchBar.addEventListener("input", async (event) => {
      ...
      if (q === "") {
        ...
        if (comedianHits.length > 0) {
          ...
          comedianCard.innerHTML = `<div class="search-header">Comedians</div>`;
        ...
        if (venueHits.length > 0) {
          ...
          venueCard.innerHTML = `<div class="search-header">VENUES</div>`;
