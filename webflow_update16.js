...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function startApp(city) {
    ...
    const data = await filterForLocation(39.963742, -82.995971, 50 * 1000);
    await filterForLocationForVenues(39.963742, -82.995971, 50 * 1000);
    populateEvents(data);
    setUpEventListeners();
  }
  ...
  async function filterEventsByVenue(selectedVenue) {
    ...
    const q = query(
      collection(db, "events"),
      where("locationName", "==", selectedVenue.venue_name),
      where("date", ">=", d),
      orderBy("date")
    );
    const querySnapshot = await getDocs(q);
